...
import com.github.xcfyl.drpc.springboot.starter.annotation.*;
 ...
 */
public class DrpcServerImportSelector implements ImportSelector, BeanFactoryAware {
    private BeanFactory beanFactory;

    @NonNull
    @Override
    public String[] selectImports(@NonNull AnnotationMetadata metadata) {
        Map<String, Object> attributes = metadata.getAnnotationAttributes(EnableDrpcServer.class.getName());
        if (attributes == null || attributes.size() == 0) {
            return new String[0];
        }

        String[] scanPackages = (String[]) attributes.get("scanPackages");
        DrpcServerScanner scanner = new DrpcServerScanner((BeanDefinitionRegistry) beanFactory);
        ...
        for (BeanDefinitionHolder holder : definitionHolders) {
            ...
            try {
                ...
                if (clazz.isAnnotationPresent(DrpcService.class)) {
